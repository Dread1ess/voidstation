...
export class PianoRoll {
  ...
  activeTrackIndex = 3; // Default to Synth track (index 3)
  gridElements: HTMLElement[][] = []; // [pitchIndex][stepIndex] -> element
  currentStep = -1;
  ...
  setActiveTrack(trackIndex: number) {
  ...
  _applyCellState(cell: HTMLElement, pitch: number, step: number) {
  ...
  _wirePianoKeys() {
    ...
    const keyRows = keysContainer.querySelectorAll<HTMLElement>('.key-row');

...
export class AudioEngine {
    ...
    deletePattern(index) {
        if (this.patterns.length <= 1 || !this.patterns[index])
            return;
    ...
    setPlaylistCell(bar, patternIndex) {
        ...
        if (patternIndex === undefined || patternIndex < 0) {
            ...
            if (this.loopEnabled && bar + 1 > this.loopEnd)
                this.loopEnd = bar + 1;
        }
        this._notifyPatternChange();
    }
    ...
    movePlaylistClip(fromBar, toBar) {
        ...
        if (this.loopEnabled && toBar + 1 > this.loopEnd)
            this.loopEnd = toBar + 1;
        this._notifyPatternChange();
    }
    ...
    _stepSourceForBar(trackIndex, bar) {
        if (this.playlist.length > 0) {
            const patIdx = this.playlist[bar];
            const pat = patIdx !== undefined ? this.patterns[patIdx] : null;
            if (pat && pat.tracks[trackIndex])
                return pat.tracks[trackIndex];
    ...
    _buildSynthVoice(ctx, track, destGain, midiNote, time, duration) {
    ...
    get hasContent() {
        return this.patterns.some(p => p.tracks.some(t => t.pattern.some(Boolean) ||
            (t.pianoGrid && t.pianoGrid.some(row => row.some(active => active))))) || this.hasSample;
    }
    ...
    setSampleTrim(trackIndex, start, end) {
        const track = this.tracks[trackIndex];
    ...
    playSample(trackIndex, offset = 0) {
        const ctx = this.ensureContext();
        if (!ctx)
            return;
        const track = this.tracks[trackIndex];
    ...
    _scheduleStep(time, trackIndex) {
        ...
        const anySolo = this.tracks.some(t => t.solo);
        if (anySolo && !track.solo)
            return; // solo active, skip non-soloed tracks
        if (track.mute)
            return; // muted tracks silent regardless
    ...
    async deserialize(state) {
        ...
        let patterns = Array.isArray(state.patterns) ? state.patterns : [];

...
export class PlaylistBar {
    constructor(engine) {
        this.container = null;
        this.cells = [];
        this.PL_BARS = 20; // keep in sync with --bars (20 bars, 160px each)
        this._currentBar = -1;
        this._loopBand = null;
        this._loopHandleStart = null;
        this._loopHandleEnd = null;
    ...
    _onCellClick(bar) {
        ...
        if (performance.now() - this._lastDragAt < 300)
            return;
    ...
    _onCellPointerDown(ev, bar) {
        if (this._dragClip)
            return; // already dragging
        const patIdx = this.engine.playlist[bar];
        if (patIdx === undefined)
            return; // only assigned clips are draggable
        ...
        const move = (me) => this._onCellDragMove(me, cell);
        const up = () => {
    ...
    _onCellDragMove(ev, cell) {
        const d = this._dragClip;
        if (!d)
            return;
        if (!d.active) {
            if (Math.hypot(ev.clientX - d.startX, ev.clientY - d.startY) < this._dragThreshold)
                return;
            d.active = true;
    ...
    _toggleDropTarget(bar, on) {
        const c = this.cells[bar]?.cell;
        if (c)
            c.classList.toggle('drop-target', on);
    }
    ...
    _attachHandleDrag(container, handle, which) {
    ...
    _syncPlayState() {
        if (!this.engine.isPlaying) {
            if (this._currentBar >= 0) {
                ...
                if (prev)
                    prev.cell.classList.remove('playing');
                this._currentBar = -1;
            }
            return;
        }
        ...
        if (bar !== this._currentBar) {
            ...
            if (cur)
                cur.cell.classList.add('playing');
